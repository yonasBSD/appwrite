...
function messagingFlow(ctx) {
    requireSession(ctx, 'messagingFlow');
    if (!ctx.userId || !ctx.userEmail) {
        throw new Error('accountFlow must run before messagingFlow');
    }
...
function detailsTable(data) {
    return [
        ...
        '| --- | ---: | ---: | ---: | ---: |',
        detailRow(data, 'HTTP total', 'appwrite_http_duration'),
        detailRow(data, 'API endpoints', 'appwrite_api_duration'),
        detailRow(data, 'Database worker schema jobs', 'appwrite_worker_database_duration'),
        detailRow(data, 'TablesDB worker schema jobs', 'appwrite_worker_tables_duration'),
        detailRow(data, 'Mail worker delivery', 'appwrite_worker_mails_duration'),
        detailRow(data, 'Messaging worker delivery', 'appwrite_worker_messaging_duration'),
    ].join('\n');
}

function detailRow(data, label, metric, unit = 'ms') {
    const values = data.metrics[metric] && data.metrics[metric].values;
    if (!values || values.count === 0) {
        return `| ${label} | n/a | n/a | n/a | n/a |`;
    }

    return `| ${label} | ${formatDetailValue(values.avg, unit)} | ${formatDetailValue(values['p(90)'], unit)} | ${formatDetailValue(values['p(95)'], unit)} | ${formatDetailValue(values.max, unit)} |`;
}
